...
class ThreeLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    f = scores - np.max(scores, axis=1, keepdims=True) ## 数据防溢出
    loss = -f[range(N), y].sum() + np.log(np.exp(f).sum(axis=1)).sum() ## cross entropy
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      if it % 10 == 0:
        ...
        if count >= 5:
          count = 0
          learning_rate = 0.5 * learning_rate  ## lr_缩减

      if verbose and it % 100 == 0:
        train_acc = (self.predict(X_batch) == y_batch).mean()
        print('iteration %d / %d: loss %f  train_acc %f'  % (it, num_iters,loss, train_acc))
